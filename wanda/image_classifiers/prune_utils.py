...
def compute_mask(W_metric, prune_granularity, sparsity, prune_nm=(0,0)):
    if prune_nm != (0,0):
        W_mask = (torch.zeros_like(W_metric) == 1)  ## initialize a mask to be all False
        prune_n, prune_m = prune_nm
        for ii in range(W_metric.shape[1]):
            if ii % prune_m == 0:
                tmp = W_metric[:,ii:(ii+prune_m)].float()
                W_mask.scatter_(1,ii+torch.topk(tmp, prune_n,dim=1, largest=False)[1], True)
    elif prune_granularity == "layer":
...
def prune_vit(args, model, calib_data, device, prune_nm=(0,0)):
    inps = calib_data 
    bs = inps.shape[0]
    require_forward = (args.prune_metric in ["wanda"])
    ...
    # for block_id, blk in enumerate(model.blocks):
    for block_id, blk in tqdm(enumerate(model.blocks), total=len(model.blocks)):
        subset = find_layers(blk)
        ...
        for name in subset:
            if args.prune_metric == "wanda":
                # print('wanda based, scale activations...')
                metric_stats[block_id][name] *= torch.sqrt(wrapped_layers[name].scaler_row.reshape((1,-1)))
            # else:
                # print('simple magnitude based...')
            W_mask = compute_mask(metric_stats[block_id][name], args.prune_granularity, args.sparsity, prune_nm=prune_nm)
