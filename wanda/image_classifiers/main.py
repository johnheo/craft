...
def main(args):
    ...
    print(args)
    print('\n======> ', args.resume, 'with sparsity ', args.sparsity, 'and prune metric ', args.prune_metric, '\n')
    ...
    dataset_train, dataset_val = load_cifar(args.data_path, 
                                        batch_size=args.batch_size, 
                                        num_workers=args.num_workers, 
                                        autoaugment=False, 
                                        data_path = '/mnt/hdd-nfs/johnheo/data'
                                        )
    ...
    # cifarmod = create_model('vit_base_patch16_224', pretrained=False, num_classes=100, img_size=32)
    model.cuda()
    if args.distributed:
    ...
    if "convnext" in args.model:
        ...
        model.load_state_dict(undo_ddp(checkpoint))
    ...
    if args.sparsity_type != "unstructured":
        ...
        pn, pm = map(int, args.sparsity_type.split(":"))
    ...
    if args.sparsity != 0:
        with torch.no_grad():
            if "convnext" in args.model:
                ...
                prune_vit(args, model, calib_data, device, prune_nm=(pn,pm))
            elif "deit" in args.model:
